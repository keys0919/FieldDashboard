...
export default async function ShareReportsPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const [{ data: activities }, { data: summaries }] = await Promise.all([
    sb.from('activities').select('id, date, participant_id, type_key, status, diary_day').eq('project_id', project.id).order('date').order('participant_id'),
    sb.from('summaries').select('date').eq('project_id', project.id),
  ])
      ...
      <div className="px-6 pb-4 max-w-3xl">
        {dates.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-on-surface-variant">
            ...
            <p className="text-sm font-medium">리포트가 없습니다</p>
          </div>
        ) : (
